...
const CompanyInfoPage: React.FC = () => {
  ...
  const [exportDataList, setExportDataList] = useState<
    { companyName: string; wikipediaText: string }[]
  >([]);
  const [noWikiFoundExportDataList, setNoWikiFoundExportDataList] = useState<
    { companyName: string }[]
  >([]);
  const [wikiFoundExportDataList, setWikiFoundExportDataList] = useState<
    { companyName: string }[]
  >([]);
  ...
  async function fetchCompanyWikipediaText(
    ...
    try {
      const response = await axios.get(
        `/api/wikipedia?companyName=${encodeURIComponent(companyName)}`
      );
        ...
        sections.each((index, section) => {
          if (containsText($(section), header)) {
            ...
            if (matchingSection.trim() == "") {
              ...
              var sectionDataWithoutReferencesAndExternalLinks =
                $content.html();
              sectionData[header] =
                sectionDataWithoutReferencesAndExternalLinks;
            } else {
              sectionData[header] = matchingSection;
            }
            if (sectionData[header]) return false; // Stop searching after finding the first occurrence
          }
        });
      });
    });
  ...
  const exportData = (
    ...
    data.forEach(({ companyName, wikipediaText }) => {
      ...
      const safeCompanyName = companyName
        ...
        : "";
      const safeWikipediaText = wikipediaText
        ? wikipediaText.replace(/"/g, '""')
        : "";
    ...
    companyNames.forEach(async (company) => {
      ...
      if (text) {
        ...
        if (Object.keys(controversySections).length > 0) {
          ...
          setExportDataList((prevData) => [
            ...prevData,
            {
              companyName: company,
              wikipediaText: combinedText.replace(/<[^>]*>?/gm, ""),
            },
          ]);
          ...
          setWikiFoundExportDataList((prevData) => [
            ...prevData,
            { companyName: company },
          ]);
        ...
        setNoWikiFoundExportDataList((prevData) => [
          ...prevData,
          { companyName: company },
        ]);
  ...
  return (
    <LayoutContainer>
      ...
      <form onSubmit={handleSubmit}>
        <p>
          Wikiscraper detects companies with Wikipedia controversies, pages, or
          no presence and exports the results.
        </p>
        <br />
        <br />
        <label>
      ...
      {wikipediaText !== null && (
        <div style={{ float: "left", clear: "both" }}>
          <br />
          <h2>
            Results: {matchCount} / {totalCount}
          </h2>
          <Button onClick={() => exportData(exportDataList)} variant="outlined">
            Export Controversy Matches ({matchCount})
          </Button>
          &nbsp;
          <br />
          <br />
          <Button
            onClick={() => exportDataNoWikiPage(wikiFoundExportDataList)}
            variant="outlined"
          >
            Export Wiki Matches but no Controversy ({wikiMatchCount})
          </Button>
          &nbsp;
          <br />
          <br />
          <Button
            onClick={() => exportDataNoWikiPage(noWikiFoundExportDataList)}
            variant="outlined"
          >
            Export Companies Without Wikipedia Page ({noWikiFoundCount})
          </Button>
          <div
            className="wikipediaResultsHolder"
            dangerouslySetInnerHTML={{ __html: wikipediaText }}
          ></div>
        </div>
      )}
    </LayoutContainer>
