...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { url, clientName, clientId, userToken, categories } = req.body;

  if (!url || !userToken) {
    return res.status(400).json({ message: 'Missing URL or User Token' });
  }

  if (!clientName && !clientId) {
    return res.status(400).json({ message: 'Either Client Name or Client ID must be provided' });
  }
  ...
  try {
    ...
    const [submission] = await connection.query('SELECT * FROM superstar_sites WHERE domain = ?', [
      fullDomain,
    ]);
    ...
    await connection.execute(
      `INSERT INTO superstar_site_submissions 
        (superstar_site_id, title, user_token, content, client_name, client_id, categories, submission_response, autogenerated) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        superstarSiteId,
        title,
        userToken,
        content,
        clientName, // Still save the client name for backward compatibility
        clientId || null, // Store the client ID if provided
        categories,
