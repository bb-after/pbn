...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (searchQuery) {
      query = 'SELECT pbn_site_submissions.*, users.name FROM pbn_site_submissions JOIN users ON users.user_token = pbn_site_submissions.user_token WHERE title LIKE ? and deleted_at is null ORDER BY pbn_site_submissions.id DESC LIMIT ? OFFSET ?';
      queryConfig = [`%${searchQuery}%`, rowsPerPage, offset];
    } else {
      query = 'SELECT pbn_site_submissions.*, users.name FROM pbn_site_submissions JOIN users ON users.user_token = pbn_site_submissions.user_token WHERE deleted_at is null ORDER BY pbn_site_submissions.id DESC LIMIT ? OFFSET ?';
