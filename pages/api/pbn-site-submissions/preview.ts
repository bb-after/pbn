...
2.  **Link Integration:** Seamlessly weave a hyperlink to the **Target URL** (${url}) into the article's content. The anchor text for this link MUST be exactly **ONE** of the provided **Keywords** (${keywordsString}). Choose the keyword that fits most naturally.
3.  **Tone and Quality:** Write in a clear, informative, and engaging tone. Ensure high-quality writing with correct grammar and spelling. Avoid overly promotional language.
4.  **Output Format:** Respond **ONLY** with a valid JSON object containing the following two keys and nothing else. Do not include any introductory text, explanations, markdown formatting, or anything before or after the JSON object itself:
